...
Channel_26_29= [T];
twods_Nt=Channel_26_29; % 3 to 50 microns
% twods_limit=10;
...
Nsumtotal3=Nsum_Nd_3./bindd';
 stairs(binmid,Nbinweird_1);
 hold all
 stairs(binmid,Nbinweird_2);
 stairs(binmid,Nsumtotal3);
%
...
% %-----Plot Cosmetics-----%
eval(sprintf('title(''2DS Cloud Droplet Number Distribution - High Sawtooth 2d Values - %6s'');',k,datestr(timevec_twods(1),2)));
xlabel('Diameter (\mum)','FontSize',20);
  ylabel('N(D) (cm^-^3 \mum^-^1)','FontSize',20);
%  ylabel('Mc(D) g \times m^-3)','FontSize',20);
legend('1st High Value Leg','2nd High Value Leg','Sawtooth3')
